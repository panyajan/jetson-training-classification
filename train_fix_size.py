...
parser.add_argument('--resolution', default='224x224', type=str, metavar='N',
...
tensorboard = SummaryWriter(
    log_dir=os.path.join(args.model_dir, "tensorboard", f"{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"))
...
def main(args):
    ...
    train_transforms = transforms.Compose([
        transforms.RandomResizedCrop(resolution_hw),
    ...
    val_transforms = transforms.Compose([
        ...
        transforms.CenterCrop(resolution_hw),
        transforms.ToTensor(),
        normalize,
    ])
...
def accuracy(output, target):
    ...
    with torch.no_grad():
        if args.multi_label:
            ...
            preds = ((
                                 output >= args.multi_label_threshold) == target.bool())  # https://medium.com/@yrodriguezmd/tackling-the-accuracy-multi-metric-9e2356f62513
